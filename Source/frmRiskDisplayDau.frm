...
Begin VB.Form frmRiskDisplayDau 
   ...
   End
End
Attribute VB_Name = "frmRiskDisplayDau"
...
Private Sub Form_Load()
  ...
  Dim C As TrueDBGrid80.Column, P As Integer, nTabsHeight As Integer, RiskNuc As Integer
  Dim firstNuc As Boolean
  ...
  Dim NumCancers As Integer, NumCols As Integer, iCol As Integer
  NumCancers = 15
  
'  On Error GoTo Error_Handler
  PositionForm Me
  nTabsHeight = RiskTab.Height - RiskTab.TabHeight - (RiskTab.TabHeight * gn_Parent) / 3
  If (nTabsHeight > 6730) Then
    nTabsHeight = 6730
  End If
  For i = 0 To gn_Parent - 1
    RiskTab.TabEnabled(i) = True
    RiskTab.TabVisible(i) = True
    grdExtDisplayDau(i).Top = 100
    grdExtDisplayDau(i).Left = 240
    grdExtDisplayDau(i).Height = nTabsHeight - 200
  Next i
  For i = gn_Parent To 19
    RiskTab.TabEnabled(i) = False
    RiskTab.TabVisible(i) = False
  Next i
  If gsRisk_Path = "Ingestion" Or gsRisk_Path = "Inhalation" Then
    Caption = gs_myTable & " risk coefficients (" & gsRisk_Unit_Int & ") from FGR 13, including daughters"
  Else
    If gs_myTable = "Ground Plane" Then
      Caption = gs_myTable & " risk coefficients (" & gsRisk_Unit_Gnd & ") from FGR 13, including daughters"
    Else
      Caption = gs_myTable & " risk coefficients (" & gsRisk_Unit_Ext & ") from FGR 13, including daughters"
    End If
  End If
  
  Data1.DatabaseName = gs_DataPath & "risk.mdb"
  
  For P = 0 To gn_Parent - 1
    
  RiskTab.TabCaption(P) = gs_NucNames(1, P + 1)
  myQuery = "Select * from FGR13CD where " & gs_MyWhereArray(P + 1)
...
'  End If
  ...
  While Not rsNuc.EOF
    If gs_myTable = "Inhalation" Then
      MaxCol(P) = MaxCol(P) + (rsNuc![InhCount] * 2)
    ElseIf gsRisk_Path = "Ingestion" Then
      MaxCol(P) = MaxCol(P) + (rsNuc![IngCount] * 4)
    Else
      MaxCol(P) = MaxCol(P) + 2
    End If
    rsNuc.MoveNext
  Wend
  If MaxCol(P) < 2 Then
    MsgBox "There are no risk coefficients for some of these nuclides."
    Me.Hide
    Exit Sub
  End If
  If gs_myTable = "Ingestion" Or gs_myTable = "Inhalation" Then
    xGrd(P).x.ReDim 0, 19, 0, MaxCol(P) + 1
    numRows = 19
  Else
    xGrd(P).x.ReDim 0, 16, 0, MaxCol(P) + 1
    numRows = 16
  End If
  ...
  While Not rsNuc.EOF
    ...
    For j = 1 To RiskCount
      ...
      If gsRisk_Path = "Inhalation" Then
        ...
        xGrd(P).x(0, iCol) = Mid(RiskRecInh, 1, 7)
        xGrd(P).x(1, iCol) = Mid(RiskRecInh, 9, 1)
        xGrd(P).x(2, iCol) = Mid(RiskRecInh, 11, 7)
      ElseIf gsRisk_Path = "Ingestion" Then
        Get #10, RiskFirst, RiskRecIng
        xGrd(P).x(0, iCol) = Mid(RiskRecIng, 1, 7)
        xGrd(P).x(2, iCol) = Mid(RiskRecIng, 9, 7)
        xGrd(P).x(1, iCol) = Mid(RiskRecIng, 16, 7)
      Else
        Get #10, RiskFirst, RiskRecE
        xGrd(P).x(0, iCol) = Mid(RiskRecE, 1, 7)
      End If
      If firstNuc Then
        xGrd(P).x(0, 0) = "Nuclide"
        If gsRisk_Path = "Inhalation" Then
          xGrd(P).x(1, 0) = "F1"
          xGrd(P).x(2, 0) = "Class"
          xGrd(P).x(4, 0) = "Cancer"
          xGrd(P).x(4, iCol) = "mortality"
          xGrd(P).x(4, iCol + 1) = "morbidity"
          xGrd(P).x(5, 0) = Mid(RiskRecInh, 19, 9) 'cancer name
        ElseIf gsRisk_Path = "Ingestion" Then
          xGrd(P).x(1, 0) = "F1"
          xGrd(P).x(2, 0) = "Intake"
          xGrd(P).x(4, 0) = "Cancer"
          xGrd(P).x(4, iCol) = "mortality"
          xGrd(P).x(4, iCol + 1) = "morbidity"
          xGrd(P).x(5, 0) = Mid(RiskRecIng, 25, 9) 'cancer name
        Else
          xGrd(P).x(1, 0) = "Cancer"
          xGrd(P).x(1, iCol) = "mortality"
          xGrd(P).x(1, iCol + 1) = "morbidity"
          xGrd(P).x(2, 0) = Mid(RiskRecE, 20, 9) 'cancer name
        End If
      Else
      ...
      End If
      If gsRisk_Path = "Inhalation" Then
        xGrd(P).x(5, iCol) = Val(Mid(RiskRecInh, 29, 9))
        xGrd(P).x(5, iCol + 1) = Val(Mid(RiskRecInh, 38, 9))
        Get #10, RiskFirst + 1, RiskRecInh
        xGrd(P).x(3, iCol) = Trim(Mid(RiskRecInh, 1, 18))
        xGrd(P).x(4, iCol) = "mortality"
        xGrd(P).x(4, iCol + 1) = "morbidity"
        xGrd(P).x(6, 0) = Mid(RiskRecInh, 19, 9) 'cancer name
        xGrd(P).x(6, iCol) = Val(Mid(RiskRecInh, 29, 9))
        xGrd(P).x(6, iCol + 1) = Val(Mid(RiskRecInh, 38, 9))
        For i = 2 To 14
          Get #10, RiskFirst + i, RiskRecInh
          If firstNuc Then
            xGrd(P).x(i + 5, 0) = Mid(RiskRecInh, 19, 9) 'cancer name
          End If
          xGrd(P).x(i + 5, iCol) = Val(Mid(RiskRecInh, 29, 9))
          xGrd(P).x(i + 5, iCol + 1) = Val(Mid(RiskRecInh, 38, 9))
        Next i
      ElseIf gsRisk_Path = "Ingestion" Then
        xGrd(P).x(5, iCol) = Val(Mid(RiskRecIng, 35, 8))
        xGrd(P).x(5, iCol + 1) = Val(Mid(RiskRecIng, 44, 8))
        Get #10, RiskFirst + 1, RiskRecIng
        xGrd(P).x(3, iCol) = Trim(Mid(RiskRecIng, 1, 18))
        xGrd(P).x(4, iCol) = "mortality"
        xGrd(P).x(4, iCol + 1) = "morbidity"
        xGrd(P).x(6, 0) = Mid(RiskRecIng, 25, 9) 'cancer name
        xGrd(P).x(6, iCol) = Val(Mid(RiskRecIng, 35, 8))
        xGrd(P).x(6, iCol + 1) = Val(Mid(RiskRecIng, 44, 8))
        For i = 2 To 14
          Get #10, RiskFirst + i, RiskRecIng
          If firstNuc Then
            xGrd(P).x(i + 5, 0) = Mid(RiskRecIng, 25, 9) 'cancer name
          End If
          xGrd(P).x(i + 5, iCol) = Val(Mid(RiskRecIng, 35, 8))
          xGrd(P).x(i + 5, iCol + 1) = Val(Mid(RiskRecIng, 44, 8))
        Next i
        ...
        xGrd(P).x(5, iCol + 1) = Val(Mid(RiskRecIng, 44, 8))
        Get #10, RiskFirst + 1, RiskRecIng
        xGrd(P).x(3, iCol) = Trim(Mid(RiskRecIng, 1, 20))
        xGrd(P).x(4, iCol) = "mortality"
        xGrd(P).x(4, iCol + 1) = "morbidity"
        xGrd(P).x(6, 0) = Mid(RiskRecIng, 25, 9) 'cancer name
        xGrd(P).x(6, iCol) = Val(Mid(RiskRecIng, 35, 8))
        ...
        Next i
      Else
        xGrd(P).x(2, iCol) = Val(Mid(RiskRecE, 29, 9))
        xGrd(P).x(2, iCol + 1) = Val(Mid(RiskRecE, 38, 9))
        For i = 1 To 14
          Get #10, RiskFirst + i, RiskRecE
          If firstNuc Then
            xGrd(P).x(i + 2, 0) = Mid(RiskRecE, 20, 9) 'cancer name
          End If
          xGrd(P).x(i + 2, iCol) = Val(Mid(RiskRecE, 29, 9))
          xGrd(P).x(i + 2, iCol + 1) = Val(Mid(RiskRecE, 38, 9))
        Next i
      End If
      iCol = iCol + 2
      RiskFirst = RiskFirst + 15
  ...
  If MaxCol(P) > grdExtDisplayDau(P).Columns.Count Then
    j = grdExtDisplayDau(P).Columns.Count
  ...
  If gs_myTable = "Ingestion" Or gs_myTable = "Inhalation" Then
    If gdRisk_Factor_Int <> 1 Then
      For j = 1 To MaxCol(P)
        For i = 5 To numRows - 1
            xGrd(P).x(i, j) = xGrd(P).x(i, j) * gdRisk_Factor_Int
        Next i
      Next j
    End If
    ...
    If gdRisk_Factor_Gnd <> 1 Then
      For j = 1 To MaxCol(P)
        For i = 2 To numRows - 1
          xGrd(P).x(i, j) = xGrd(P).x(i, j) * gdRisk_Factor_Gnd
        Next i
      Next j
    End If
  Else
    If gdRisk_Factor_Ext <> 1 Then
      For j = 1 To MaxCol(P)
        For i = 2 To numRows - 1
          If (IsNumeric(xGrd(P).x(i, j))) Then
            xGrd(P).x(i, j) = xGrd(P).x(i, j) * gdRisk_Factor_Ext
          End If
        Next i
      Next j
    End If
  ...
  grdExtDisplayDau(P).Columns(0).Width = 1000
